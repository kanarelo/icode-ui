...
import com.icode.view.component.fields.ComboBox.Item;
...
public class PopupDialog extends JPanel {
     ...
     * @param items
     */
    public PopupDialog(String title, String message, SearchWizard sw, Item... items) {
        this();
        add(createFooter("Search", "Cancel"), BorderLayout.SOUTH);
        add(createSearchDialogHeader(title, message, sw, items), BorderLayout.NORTH);
    }
    
    public PopupDialog(String title, JPanel formContainer) {
    	this(title, formContainer, false);
    }
    
    public PopupDialog(String title, JPanel formContainer, boolean translucent) {
    	this();
		this.translucent = translucent;
    ...
    private JPanel createSearchDialogHeader(String title, String message, final SearchWizard sw, Item... items) {
        JPanel header = new JPanel(new BorderLayout(8,0));
        header.setBorder(BorderFactory.createEmptyBorder(8, 0, 8, 0));

        header.add(new JLabel(ResourceUtils.getIcon("/icons/48/bSearch.png")), BorderLayout.WEST);
                ...
                if (txtSearch.getText() != null) {
                    if (!txtSearch.getText().isEmpty()) {
                        if(cmbColumn.getSelectedIndex() != -1){
                            sw.setSearchItem(txtSearch.getText());
                            sw.setField(cmbColumn.getContent());
                        }
                    }
                }
            }
    ...
    public PopupDialog(Throwable exception) {
        ...
        this.messageType = (exception instanceof Error) ? MessageType.ERROR : MessageType.WARNING;
        add(createHeader(messageType, root.getClass().getName(), root.getLocalizedMessage()),
                BorderLayout.NORTH);
    ...
    private JPanel createHeader(MessageType messageType, String title, String message) {
        JPanel header = new JPanel(new BorderLayout(8, 0));
        header.setBorder(BorderFactory.createEmptyBorder(8, 0, 8, 0));
        ...
        if (messageType != null){
	        if (messageType != MessageType.PLAIN_MESSAGE) {
	            header.add(
	                    new JLabel(UIManager.getIcon("OptionPane."
	                    + ((messageType == MessageType.ERROR) ? "error"
	                    : (messageType == MessageType.QUESTION) ? "question"
	                    : (messageType == MessageType.WARNING) ? "warning" : "information")
    ...
    private JPanel createFooter(String ok, String cancel) {
        ...
        footer.setBorder(new LineBorder(Color.LIGHT_GRAY, 1, 0, 0, 0, 8, 0, 0, 0));
