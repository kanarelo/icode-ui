...
public class MenuBar extends JToolBar {
    ...
    private static class StringToolButton extends PressButton {
		...
		protected void paint(Graphics2D g, boolean inside, boolean pressed, boolean focused) {
            w = getWidth();
            h = getHeight();
            gpOver = new GradientPaint(0, 0, new Color(0xFFF5CC), 0, h * .75F, new Color(0xFFDB75));
            if (isEnabled()) {
                if (pressed || inside) {
                    if (inside && !pressed) {
                        g.setPaint(gpOver);
                ...
                if (popup != null) {
                    if (popup.isVisible()) {
                        ...
                        g.setPaint(new GradientPaint(0, 0, new Color(232, 233, 241), 0, h * .75F, new Color(177, 176, 198)));
                        g.fillRect(1, 1, getWidth() - 2, getHeight() - 2);
                        g.setColor(Color.BLACK);
                        //g.setFont(getFont().deriveFont(Font.BOLD, getFont().getSize() * .86F));
                        if (!popupSelected) {
                            popupSelected = true;
                        }
                    } else {
                ...
                g.drawString(label, (getPreferredSize().width / 7.5F), (getPreferredSize().height / 1.5F));
            }
        }
        ...
        public Dimension getPreferredSize() {
            ...
            return new Dimension(fm.stringWidth(label) + 15, this.getParent().getHeight());
        }
     ...
     * @return the created button component
     */
    public PressButton addButton(Class<?> clazz, String path, String tip, JPopupMenu popup) {
    ...
    public static class ToolButton extends PressButton {
		...
		protected void paint(Graphics2D g,
            ...
            w = getWidth();
            h = getHeight();
            gpOver = new GradientPaint(w * .5f, 0, new Color(255, 224, 102), w * .5f, h * 2, new Color(252, 249, 223), true);
            gpPressed = new GradientPaint(0, 1, Color.white, 0, h, new Color(250, 233, 166));
            if (isEnabled()) {
                if (pressed && inside) {
            ...
            g.setColor(isEnabled() ? inside ? Color.darkGray : Color.gray : Color.lightGray);
            icon.paintIcon(this, g, (int) ((this.getWidth() / 2.0) - (this.icon.getIconWidth() / 2.0)),
                    (int) ((this.getHeight() / 2.0) - (this.icon.getIconHeight() / 2.0)));

        }

        @Override
        public Dimension getPreferredSize() {
            return new Dimension(Math.max(22, icon.getIconWidth() + 6), Math.max(22, icon.getIconHeight() + 6));
        }
