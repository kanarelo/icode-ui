...
public class ToolFooter extends JPanel {
     ...
     * @param icon the icon
     * @param tootTip the tooltip text for the button
     * @param listener the action listener listening for the button press
     * @return the created component
     */
    public JComponent addButton(Icon icon, String tootTip, ActionListener listener) {
     ...
     * @return the created component
     */
    public JComponent addMinusButton(String tootTip, ActionListener listener) {
        return addButton(new MinusIcon(), tootTip, listener);
    }

    public JComponent addExpandButton(String tootTip, ActionListener listener) {
    ...
    protected void paintComponent(Graphics g) {
        Insets is = getInsets();
        int x = is.left, y = is.top, w = getWidth() - is.left - is.right,
                h = getHeight() - is.top - is.bottom, h2 = h / 2;

        g.setColor(new Color(0xeeeeee));
        ((Graphics2D) g).setPaint(new GradientPaint(w * .5F, 0, new Color(230, 234, 238), w * .5F, h, new Color(191, 195, 200)));
        g.fillRect(x, y, w, h);
    }
    ...
    private static class ToolButton extends PressButton {
		...
		protected void paint(Graphics2D g, boolean inside, boolean pressed, boolean focused) {
            w = getWidth();
            h = getHeight();
            gpOver = new GradientPaint(w * .75F, 0, new Color(255, 224, 102), w * .5F, h * 3, new Color(252, 249, 223), true);
            g.setColor(new Color(0xaaaaaa));
            g.fillRect(getWidth() - 1, (int) (getHeight() * .25F), 1, (int) (getHeight() * .5F));
            if (isEnabled()) {
                if (pressed && inside) {
            ...
            icon.paintIcon(this, g, (getWidth() - 1 - icon.getIconWidth()) / 2, (getHeight() - icon.getIconHeight()) / 2);
        }
