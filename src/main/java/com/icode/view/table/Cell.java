 ...
 * representation. Note: it is under construction, it'll be radically changed.
 */
public class Cell {
	 ...
	 * 
	 * @param name
	 *            the name of the getter (and for editable cell setter) method
	 */
	...
	public Cell setMessageFormat(MessageFormat format) {
		return setFormat(format);
	}

	public Cell setStringFormat(String formatPattern) {
		return setFormat(new MessageFormat(formatPattern));
	}
	...
	void render(JComponent tableRenderer, Graphics2D g, Object item,
		...
		if (format == progress) {
			int value = Math.max(0,
					Math.min(((Number) getValue(item)).intValue(), 100));
			int w = Math.min(width, 80), length = value * w / 100, h = 7, py = y
					+ (height - h) / 2;
			...
			if (length < w) {
				...
				g.setPaint(new GradientPaint(0, py + 1, new Color(0xc0c0c0), 0,
						py + h - 2, new Color(0xe0e0e0)));
			...
			if (length > 0) {
				...
				g.setPaint(new GradientPaint(0, py + 1, new Color(0xf0c070), 0,
						py + h - 2, new Color(0xf07030)));
			...
			g.drawString(text, fx,
					y + (height + fm.getAscent() - fm.getDescent() + 1) / 2);
		}
	}
