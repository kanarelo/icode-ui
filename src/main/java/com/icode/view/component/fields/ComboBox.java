...
public class ComboBox extends JComboBox
     ...
     * @param value a value binded to the row, can be null too
     * @param title the main text
     * @param decription the second auxiliary text
     * @return the component itself
     */
    public ComboBox add(Object value, String title, String decription) {
        ((DefaultComboBoxModel) getModel()).addElement(new Item(value, title, decription));
        setSelectedIndex(-1);
        return this;
    }
     ...
     * @param required true for non empty selection requirement
     */
    ...
    /**
     * Changes the component drawn without border
     */
    ...
    /**
     * Changes selection
     * @param value an item's binded value object
    ...
    /**
     * Checks the validity of the selection, empty selection for required component is invalid
    ...
    private class Renderer extends DefaultListCellRenderer implements ItemListener {
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            return super.getListCellRendererComponent(list, null, index, selected, focused);
        }
        ...
        public Dimension getPreferredSize() {
            ...
            if (index == -1) {
                return new Dimension(
                        is.left + is.right + pfm.stringWidth(item.title) + 1/*mac*/,
                        is.top + is.bottom + pfm.getHeight());
            }

            FontMetrics sfm = getFontMetrics(small);
            return new Dimension(
                    is.left + is.right + Math.max(pfm.stringWidth(item.title), sfm.stringWidth(item.decription)) + 8,
                    is.top + is.bottom + pfm.getHeight() + sfm.getAscent() + sfm.getDescent() + 4);
        }
        ...
        protected void paintComponent(Graphics g) {
            ...
            if (index == -1) {
                g.drawString(item.title, is.left, (getHeight() + is.top - is.bottom
                        + pfm.getAscent() - pfm.getDescent() + 1) / 2);
            } else {
                g.drawString(item.title, is.left + 4, is.top + pfm.getAscent() + 2);
                g.setFont(small);
                FontMetrics sfm = g.getFontMetrics();
                if (!selected) {
                    g.setColor(Color.darkGray);
                }
                g.drawString(item.decription, is.left + 4, is.top + pfm.getHeight() + sfm.getAscent() + 2);
            }
        }
...
//        }
    ...
    public void paint(Graphics g) {
        if (hoverStyle) {
            ...
            g.fillPolygon(new int[]{x, x + 8, x + 4}, new int[]{y, y, y + 8}, 3);
        } else {
            super.paint(g);
        }
    }
